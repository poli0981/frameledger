...
CREATE TABLE games (
  ...
  ac_exception_checked_block TEXT,
  ac_exception_at INTEGER,                     -- the user's grant; NULL = no exception in force
  ac_exception_family TEXT,                    -- the family it covers (kept as history once it ends)
  ac_exception_disclosure_version TEXT,        -- AntiCheatExceptionDisclosure.Version the user accepted
  ac_exception_exe_size_bytes INTEGER,         -- the executable the grant was made on; other bytes end it
...
> key, the user's grant, and how the last grant ended — and `sessions.ac_exception_family`, the family a HOOKED session
> ran under the exception with. `ADD COLUMN` only; no row changes, so a blocked row stays blocked, not eligible, with
> no grant, until the Agent and the user say otherwise (`19_SAFETY` §The user-mode exception). Written through
> `IGameConsentStore` only — `RecordExceptionEligibilityAsync` (the sweep), `GrantAntiCheatExceptionAsync` (the
> `SetAntiCheatException` command; the store applies `UserModeExceptionRules.IsEligible` itself), `RevokeAntiCheatExceptionAsync`
