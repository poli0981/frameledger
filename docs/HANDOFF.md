...
- **D8 — FR-11 is NOT a consent precondition in P2.** `ILegalAcceptanceStore` (read-only ~~until P3~~ — the writer exists since P3 PR-3, the gate that calls it is PR-9's) is declared
...
2. ~~**FR-2.1's consent dialog, which retires `ConsentProvenance.AgentConsoleOperator`.**~~ **LANDED 2026-09-13 (PR-4)** — the dialog, the `Shared` family, `ConsentProvenance.ConsentDialog` and the Agent's stamp; the console verb is not retired but demoted to the developer path (`19_SAFETY` §User-facing consent). The paragraph below is kept as the reasoning that held until then. The console
   verb exists because the reviewed wording does not: `09_I18N` reviews `Safety_*` strings as legal
   text in en/vi/ja, and ~~no `.resx` exists anywhere in this tree~~ (the App's shell family exists
   since PR-2, 2026-09-13, under the audit; `Shared`'s `Safety_*` family is still this item's). The
   dialog is the first thing that needs a REVIEWED one. When it lands, the enum gains its FR-2.1 member, the console verb becomes the
...
| ~~1b~~ | ~~**The pipe, command half**: `SetWatchlist` (identity only), `SetHookEnabled` (the Agent re-scans and stamps), `LaunchGame`, `PauseCapture`/`ResumeCapture`, `StopSession`, `UpdateRules` (a trigger, no payload), `Shutdown`~~ **LANDED 2026-09-13** — status in `CHANGELOG.md`; `07_IPC` §Messages carries each row as built. **One thing stayed with PR-4 on purpose:** `SetHookEnabled true` stamps nothing — the pre-scan runs and a block is recorded, then `Error DisclosureUnavailable` — because the reviewed text it would stamp against does not exist, and a member no code can produce is the §S29(c) shape | ~~the Agent accepts no command from outside; `CaptureOrchestrator` has no stop/pause surface; `SetHookEnabled` has no handler~~ |
| ~~2~~ | ~~**resx + the App shell**: `Strings.resx` en/vi/ja for App and Shared, the generated class, `tools/resx-audit.ps1` (+`-SelfTest`) as `build.ps1` step 15; the Generic Host bootstrap, `FluentWindow`, TitleBar/Menu/NavigationView, theme + persistence, `INavigationService`, Serilog `ui-*.log`, five empty pages, the Agent status pill over `Hello`, auto-start of `--serve` on connect failure; `tests/FrameLedger.App.Tests`~~ **LANDED 2026-09-13** — status in `CHANGELOG.md`; `08_UI` §Shell and `16_WPFUI_SYNTAX` §App bootstrap carry what was built and the deviations. **Two things moved on purpose:** `Shared`'s family waits for PR-4 (it would hold only `Safety_*`, and an empty family is what the audit refuses), and the generator is ours (`tools/resx-gen.ps1`, committed output) because the SDK-style WPF project runs no `ResXFileCodeGenerator` | ~~`find . -name '*.resx'` is empty; step 15 prints "skipped loudly"; `App.xaml` uses `StartupUri` and `MainWindow` is a plain `Window`~~ |
...
| ~~3~~ | ~~**Ports the UI reads and writes**: `ISessionRepository.ListByGameAsync` / segments / sensors, `ISessionAnnotationRepository`, `ILegalAcceptanceStore.RecordAsync`, `IGameRepository.UpdateMetadataAsync` / `RemoveAsync`, the **settings registry** (closes §G's row — D16 below), `IRtOverrideRepository` (FR-8.3), `display_res`/`display_hz` producer; tests for the four PR-B adapters that have none~~ **LANDED 2026-09-13** — status in `CHANGELOG.md`; `06_DATA_MODEL` §Settings registry, §Writer ownership and §Migrations (schema 0002) carry what was built. **One deviation on purpose:** there is no `IRtOverrideRepository` — the FR-8.3 per-session overrides are columns on `session_annotations` (the UI's table) written through `ISessionAnnotationRepository`, and the game defaults through `IGameRepository.SetTriStateDefaultAsync`, because an override on `sessions` would have been the UI writing the Agent's table; `Application.TriState.TriStateResolution` is the precedence (manual → measured → inherited → N/A) | ~~no port lists sessions by game; `session_annotations` has no repository; `legal_acceptance` has no writer; `ISettingsStore`'s own doc calls the registry open~~ |
| ~~4~~ | ~~**FR-2.1's consent dialog**: `Safety_*` en + vi (ja marked `safety: human review required`, shown as en) in `Shared`, `SafetyDisclosureVersion`; `ConsentProvenance.ConsentDialog` whose only producer is the Agent's `SetHookEnabled` handler; `HelloAck.disclosureVersion` and the App **refusing to open the dialog** on mismatch (§S23-1's shape); the dialog itself per `19_SAFETY` §User-facing consent; FR-2.2's disabled toggle with the reason inline; FR-2.5's InfoBar; `GameConsentRecordTests`, `19_SAFETY`, `09_I18N` and item 2 above updated~~ **LANDED 2026-09-13** — status in `CHANGELOG.md`; `19_SAFETY` §User-facing consent carries what was built. **Two things wait for PR-5 on purpose:** FR-2.2's disabled toggle and FR-2.5's InfoBar are the game page's controls, and the page does not exist yet — their text (`Safety_Blocked_Toggle_Format`, `Safety_AutoDisabled_Format`, `Safety_Refused_*`, `Safety_Unhooked_Format`, `Safety_RecordWithoutMeasuring`) shipped here so the reviewed family is complete before anything renders it | ~~`Enum.GetNames<ConsentProvenance>()` has three names; no `Safety_*` key exists; `19_SAFETY` still carries the console exception~~ |
| ~~5~~ | ~~**Games + Dashboard**: the grid, game detail (header, **Supports** vs **Last session measured**, the hooking control → PR-4's dialog, the Sessions tab), Dashboard (Agent status + capability badges, the live card over `SessionProgress`, recent sessions, totals), FR-1.1 add / FR-1.4 remove; `FpsReadout` and `TriStateChip`~~ **LANDED 2026-09-14** — status in `CHANGELOG.md`; `08_UI` §Dashboard and §Games carry what was built. **Deferred on purpose:** the card sparkline and the chip override flyout (PR-6, with the charts), the capability badges and Repair on the Agent card (PR-8, with the Agent section), a dropped `.lnk` (needs the shell-link reader) | ~~no page binds a `SessionRow`; nothing renders `62 → 118 FPS (×1.9 FG)`; `SessionProgress` has no consumer~~ |
...
- **D12 — commands join as one more producer.** A `Channel<AgentCommand>` with a single consumer
  ...
  trust boundary). *(As built 2026-09-13, PR-1b: **no `Channel` and no `PipeCommandHostedService`** — the
  handler runs on the pipe client's reader task and the orchestrator's running-session table gained one lock;
  two commands did not justify a queue and a second place a start can wait. The pause is `CapturePause`, read by
  the loop at each tick and told to the ring once per transition. The stop is the session's own token in
  `RecordRequest`, ending the loop as `StoppedByUser`. The stamp branch is PR-4's, exactly as written above;
  until then it answers `DisclosureUnavailable`.)*
...
- **D14 — `Safety_*` lives in `FrameLedger.Shared`, once.** Both processes reference the same resource;
  ...
  **Built in PR-4 (2026-09-13)** as `Shared.Safety.SafetyDisclosure.Version = consent-dialog/1`; the Agent side
  also refuses (`Error DisclosureVersionMismatch`) so neither side's check is the only one.
...
- **D16 — the settings registry** (closes §G's row, written into `06_DATA_MODEL` in PR-3): `ui.language`,
  ...
  the UI owns the rest. **Built in PR-3 (2026-09-13)** as `06_DATA_MODEL` §Settings registry; two things it
  settled that this bullet did not say: `retention.raw_sessions_per_game = 0` is unlimited (a sweep that keeps
  everything, never one that keeps nothing), and an operator's bounded `--seconds N` keeps its bound under any
  `capture.min_session_s`.
