...
- **The user's bypass (2026-09-21) has its own proofs, and they are about what it must NOT do.** Native (`[bypass]` in `guard_test.cpp`): a blocklisted target is refused by the plain entry and injected by the acknowledged one with the same family and signal and a verdict that is not `kAllow`; a clean target is a plain allow; a foreign or absent payload is still refused and nothing is loaded; `IsGuardJudgement` is exactly its list. Managed: `GuardMirrorTests` holds the C# list against the native one for every reason; `HookedCaptureGateTests` proves the kill switch, an un-enabled game and a missing consent still refuse with the guard never asked, that an acknowledged request never takes the plain entry (and the reverse), and that a guard which does not know the bypass is the hard gate (the port's default implementation); `HookRequestFromConsentTests` that the fifth input needs a stored record naming its disclosure and the same executable; `GuardSupervisorTests` that a judgement is counted, kept and overruled while a fact still unhooks; `SqliteGuardBypassStoreTests` that it cannot exist for a game nobody added or another binary, touches neither block nor consent, and ends with consent or the executable; `AgentCommandHandlerTests` that the pipe re-checks everything; `GameDetailViewModelTests` that a declined disclosure sends nothing and the switch shows the row.
