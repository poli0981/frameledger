...
| `SetGuardBypass` | UI→A | `{ gameId, enabled, disclosureVersion? }` → `GuardBypassAck { gameId, enabled, outcome }` (owner decision 2026-09-21, `19_SAFETY` §The user's bypass). `enabled: true` records that the user accepted the bypass disclosure for this game's executable: refused with `DisclosureVersionMismatch` for any version but the Agent's own `GuardBypassDisclosure.Version`, `ExecutableUnreadable`, `UnknownGame`, or `DisclosureUnavailable` where no store was composed; `outcome` is the store's `ConsentWriteOutcome` (`StaleFingerprint` when the binary is not the row's). `enabled: false` withdraws it. It writes two columns and enables nothing. **There is no global form of this message.** `HookEnabledAck.prescan` gains the value `"bypassed"`: the pre-scan refused, the finding was written, and the consent was stamped beside it |
