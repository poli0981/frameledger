...
| **Pipe/shm security** | Stated as policy ("DACL granting the current user's SID") but not as implementable SDDL, nor who creates the objects with what rights. The shm half is built (`fl_shm_host.h`, the Overlay creates the section under the current user's SID); the pipe half is **HANDOFF §P3 decision D11** (2026-09-13) and lands with P3 PR-1 | `07_IPC` |
