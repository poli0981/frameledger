...
~~Static rules engine + `detection-rules.json` (engines, platforms, capabilities, **anticheat blocklist**) + validator + fixtures~~ **landed 2026-09-14 (P4 PR-1: the engine existed since P0; the Agent's `DetectionSweep` is its first caller, `ApplyDetectionAsync` its first writer — HANDOFF §P4)**; store auto-import; ~~capability-vs-measured UI separation~~ **(PR-1: the Supports row reads the sweep's flags)**; ~~i18n en/vi/ja; Legal Gate;~~ Velopack + updater (with the hooked-session deferral); bug bundle incl. overlay logs; ~~tray + toasts; Settings;~~ DB maintenance; accessibility pass; the scheduled task (`--install-task`) and the Velopack install hooks. *(The struck items moved to P3 on 2026-09-13 — see §P3.)*
