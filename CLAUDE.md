...
2. **The anti-cheat guard is a hard gate, not a warning.** Before injecting, scan the target for known anti-cheat/anti-tamper modules (`19_SAFETY`). If any is found → **refuse**, log, tell the user why. ~~There is no override switch, no "I understand, continue anyway" button. Do not add one.~~
   ...
   > **Amended 2026-09-21 (owner decision): there is exactly ONE override, and its shape is the rule now.** The owner asked for a "bypass the guard" mode at the user's own risk. It exists (`docs/19_SAFETY_AND_ANTICHEAT.md` §The user's bypass) and everything about it is a constraint a PR may not loosen:
   > - **Per game, OFF by default, never global.** There is no settings key, no CLI switch, no environment variable, no field in `rules/detection-rules.json` (its schema still makes one a validation error) and no "apply to all". Do not add one.
   > - **A recorded human act.** It is turned on only through its own disclosure — which states the ban risk and that the user carries the entire risk and the developers none — accepted by BOTH a ticked acknowledgement and the typed phrase, stamped by the Agent with the disclosure's version, keyed on the executable's fingerprint like consent, and gone when hooking is turned off or the executable changes.
   > - **It overrules the guard's JUDGEMENT and nothing else.** The kill switch, a game that was merely added, a missing consent, a foreign payload, a 32-bit target and a failed injection refuse exactly as before. The guard still runs every check, still reports what it found, and the verdict is `AllowedUnderUserBypass` — never `Allow`. A Vulkan title the guard refuses stays refused.
   > - **Rule 3 is untouched, and that is the line.** The bypass changes WHETHER FrameLedger injects, never HOW: the same documented `LoadLibraryW`, the DLL's real name, exports and version info, in one chokepoint file. Nothing about it makes FrameLedger harder for anti-cheat to see, and a PR that did would still be rejected on principle.
   > - **It is visible afterwards.** A session that started under it is marked on its row, with what the guard found, on every surface and in every export.
