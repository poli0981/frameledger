...
// THE USER'S BYPASS (owner decision 2026-09-21). Four properties, each one a way this could have been built wrong:
// the unacknowledged entry point is unchanged by it; the acknowledged one still RUNS the checks and still REPORTS what
// they found; it overrules a judgement and nothing else; and the result is never kAllow.
TEST_CASE(
    "the user's bypass: a blocklisted target is refused without the acknowledgement and injected with it, and the "
    "verdict still names what was found",
    "[guard][inject][bypass]") {
    Child        child;
    std::wstring cmd = std::wstring(L"\"") + FL_HARNESS_EXE + L"\" --hold 30";
    STARTUPINFOW si{};
    ...
    // Without the acknowledgement: the hard gate, exactly as it always was.
    const Verdict refused = GuardedInjectWithSources(child.pi.dwProcessId, FL_OVERLAY_DLL, FakeSources());
    REQUIRE(refused.reason == Reason::kBlockedModule);
    CHECK_FALSE(TargetHasModule(child.pi.dwProcessId, L"FrameLedger.Overlay.dll"));

    // With it: injected, and the verdict is neither an allow nor silent about the finding.
    const Verdict under = GuardedInjectAcknowledgedWithSources(child.pi.dwProcessId, FL_OVERLAY_DLL, FakeSources());
    INFO("reason " << ReasonName(under.reason) << " family=" << under.family << " signal=" << under.signal);
    REQUIRE(under.reason == Reason::kAllowedUnderUserBypass);
    CHECK_FALSE(under.Allowed());    // a caller that only knows Allowed() still does not proceed
    CHECK(std::string(under.family) == std::string(refused.family));
    CHECK(std::string(under.signal) == std::string(refused.signal));
    CHECK(under.signal[0] != '\0');
    CHECK(TargetHasModule(child.pi.dwProcessId, L"FrameLedger.Overlay.dll"));
}

TEST_CASE("the user's bypass: a clean target is a plain allow, so the acknowledgement changes nothing where there is "
          "nothing to overrule",
          "[guard][inject][bypass]") {
    Child        child;
    std::wstring cmd = std::wstring(L"\"") + FL_HARNESS_EXE + L"\" --hold 30";
    ...
    const Verdict v = GuardedInjectAcknowledgedWithSources(child.pi.dwProcessId, FL_OVERLAY_DLL, FakeSources());
    REQUIRE(v.Allowed());
    CHECK(TargetHasModule(child.pi.dwProcessId, L"FrameLedger.Overlay.dll"));
}

TEST_CASE("the user's bypass overrules a judgement and nothing else: a foreign or absent payload is still refused, and "
          "nothing is loaded",
          "[guard][inject][bypass]") {
    Child        child;
    std::wstring cmd = std::wstring(L"\"") + FL_HARNESS_EXE + L"\" --hold 30";
    ...
    // A real file that is not ours: kernel32 exists on every machine and is nobody's Overlay.
    wchar_t foreign[MAX_PATH]{};
    REQUIRE(GetSystemDirectoryW(foreign, MAX_PATH) != 0);
    const std::wstring kernel32 = std::wstring(foreign) + L"/kernel32.dll";
    Sources            notOurs = FakeSources();
    notOurs.PayloadIsOurOwn = [](const wchar_t*, bool* ours) -> Collected {
        *ours = false;
        return Collected::kOk;
    };
    CHECK(GuardedInjectAcknowledgedWithSources(child.pi.dwProcessId, kernel32.c_str(), notOurs).reason ==
          Reason::kPayloadNotOurs);
    CHECK(GuardedInjectAcknowledgedWithSources(child.pi.dwProcessId, L"C:/no/such/payload.dll", FakeSources()).reason ==
          Reason::kInjectionFailed);
    CHECK(GuardedInjectAcknowledgedWithSources(child.pi.dwProcessId, nullptr, FakeSources()).reason ==
          Reason::kInjectionFailed);
    CHECK_FALSE(TargetHasModule(child.pi.dwProcessId, L"FrameLedger.Overlay.dll"));
}

TEST_CASE("IsGuardJudgement names the refusals a user may overrule, and only those", "[guard][bypass]") {
    for (int i = 0; i < static_cast<int>(Reason::kCount); ++i) {
        const auto r = static_cast<Reason>(i);
        const bool judgement = IsGuardJudgement(r);
        INFO("reason " << ReasonName(r));
        switch (r) {
        case Reason::kAllow:
