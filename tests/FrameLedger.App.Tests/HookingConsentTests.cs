...
public sealed class HookingConsentTests
{
    ...
    public async Task ARefusalIsARefusalWithItsSignalAndAStoreOutcomeOtherThanWrittenIsAFailure()
    {
        ...
        refused.RefusalText().Should().Contain("eac").And.Contain("EasyAntiCheat_EOS.dll").And.Contain("guard-bypass switch", "the refusal names the one way past it, and whose risk that is (2026-09-22; it said \"no way to override\" until the bypass existed)");
